...
int mali_platform_device_init(struct platform_device *pdev)
{
	...
	printk("to add platform_specific_data to platform_device_of_mali.");
	err = platform_device_add_data(pdev,
				       &mali_gpu_data,
				       sizeof(mali_gpu_data));
	if (err) {
		printk("fail to add platform_specific_data. err : %d.", err);
		goto add_data_failed;
	}

	err = rk_context_init(pdev);
	if (err) {
		printk("fail to init rk_context. err : %d.", err);
	...
	if (err) {
		printk("fail to init simple_power_model, err : %d.", err);
